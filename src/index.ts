...
for (const folder of commandFolder) {
  ...
  const commandFiles = readdirSync(path.join(__dirname, "commands", folder)).filter((file) => file.endsWith(".ts") || file.endsWith(".js")) // Filter for .ts or .js files

  for (const file of commandFiles) {
    // Dynamically import each command file
    const command = require(path.join(__dirname, "commands", folder, file)).default
    logMessage(`Loading command ${command.data.name} from ${file}`, "SUCCESS")

    if (!command.data.name) {
      logMessage(`Command file ${file} is missing name property`, "WARNING")
      continue
    }
...
client.once("ready", async () => {
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    try {
      ...
      logMessage(`[ERROR] Error executing command ${interaction.commandName}: ${error}`, "ERROR")
    }
  })
})
